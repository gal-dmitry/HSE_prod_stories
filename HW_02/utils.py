...
History plots
"""
def global_trend(_df, show_deals=True, figsize=12):
    
    title = f"[Type: {np.unique(_df.trading_type)[0]}] [Platform: {int(np.unique(_df.platform_id)[0])}]"
...
def global_trend_gaps(_df, show_deals=False, figsize=12):
...
Separate sessions
"""
def _all_sessions(_df, show_deals=True, figsize=12):
...
def groupby_session_id(df):
    ...
    # вспомогательные поля для построения графиков
    new_df = add_xs_ys(new_df)
